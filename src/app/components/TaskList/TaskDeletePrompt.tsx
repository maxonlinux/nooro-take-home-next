...
const CheckIcon = () => {
  return <span className="material-symbols-rounded text-xl">check</span>;
};
...
const TaskDeletePrompt = ({ onDelete }: TaskDeletePromptProps) => {
  ...
  return (
    <div className="absolute left-0 top-0 size-full z-10 p-4 pointer-events-none">
      <div
        className={`absolute left-0 top-0 flex items-center justify-center size-full z-20 bg-background transition-opacity p-4 pr-14 ${
          showDeletePrompt ? "opacity-100 pointer-events-auto" : "opacity-0"
        }`}
      >
